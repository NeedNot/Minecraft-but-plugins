    ...
    public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
        if (args[0].equals("start")) {
            ...
            for (Player player : Bukkit.getOnlinePlayers()) {
                player.sendMessage("§cA random slot from your inventory will be removed when you take damage!");
            }
        }
        else {
            if (args[0].equals("stop")) {
                for (Player player : Bukkit.getOnlinePlayers()) {
                    player.sendMessage("§aA random slot from your inventory will no longer be removed when you take damage");
                }
